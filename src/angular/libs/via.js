...
function project_save_with_confirm() {
    ...
    let res = project_save_confirmed(input);
    return res;
}
...
function _via_show_img(img_index) {
    ...
    for(let i = 0; i < _via_image_id_list.length; i++) {
        let imgElement = document.getElementById(`fl${i}`);
        if (imgElement) {
    ...
    if (_via_buffer_img_index_list.includes(img_index)) {
        ...
        _via_show_img_from_buffer(img_index).then(function (ok_img_index) {
            ...
            Promise.all(_via_preload_img_promise_list).then(function (values) {
                ...
                let imgElement = document.getElementById(`fl${img_index}`);
                if (imgElement) {
                    imgElement.style.backgroundColor = '#F1F5F9';
                    imgElement.style.border = '2px solid #E6E7E9';
                }
            });
